...
function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8081/api/auth/login", // ✅ fixed port
      ...
      if (response?.data?.message?.toLowerCase() === "login successful!") {
        ...
        const roleFromResponse =
          response?.data?.role?.toLowerCase() || formattedRole;
        ...
        setErrorMessage(
          response?.data?.message || "🚫 Invalid credentials or role."
        );
      }
    } catch (error) {
      ...
      if (error.code === "ECONNABORTED") {
        ...
        setErrorMessage(
          "❌ Cannot connect to server. Ensure the backend is running on port 8081."
        );
      } else {
        setErrorMessage(
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-br from-white-900 via-blue-800 to-red text-white px-4">
      <div className="bg-gray-950 p-10 rounded-2xl shadow-2xl w-full max-w-md transition duration-300 ease-in-out transform hover:scale-[1.01]">
        <h2 className="text-4xl font-extrabold text-center mb-8 text-white-400 drop-shadow-md">
          Login  
        </h2>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <div>
            ...
            <select
              ...
              value={role}
              required
              className="w-full p-3 rounded-lg bg-blue-800 text-white focus:outline-none focus:ring-2 focus:ring-green-500 transition"
          ...
          <button
            type="submit"
            className={`w-full bg-green-500 hover:bg-blue-600 active:bg-green-700 py-3 rounded-lg font-semibold text-white transition duration-200 ease-in-out ${
